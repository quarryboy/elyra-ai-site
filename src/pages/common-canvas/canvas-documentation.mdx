...
The Elyra test harness (http://ibm.biz/elyra-canvas) allows consumers to explore a number of configurations of both the canvas as well as properties.
...
2. A regular JavaScript object called <a href="canvas-controller/canvas-controller-api">CanvasController</a>. The CanvasController routes calls from the host application and calls made due to user interactions into the system. In particular it updates the internal ObjectModel which stores:
...
Check <a href="/common-properties/styling">this section</a> to find info on what CSS to include in your application's CSS. 
...
Common canvas has five optional configuration objects: config, toolbarConfig, notificationConfig, contextMenuConfig and keyboardConfig. They are documented <a href="config/canvas-config">here</a>.
...
The first technique is provided by Common canvas. The second requires some development work which is documented <a href="enabling-external-node-creation">here</a>. 
...
Your application can disable any or all of these actions by providing the <a href="/common-canvas/config/keyboard-config">keyboard config object</a> to the CommonCanvas react component.